        ...
        this.repairButton = new Button() {
            ...
            public void onClickedInMenu(final Player player, final Menu menu, final ClickType click) {
                ...
                if (Settings.TurretSection.DISPLAY_HOLOGRAM) {
                    final List<String> loreHologram = Lang.ofList("Turret_Display.Hologram", "{turretType}", TurretUtil.capitalizeWord(turretData.getType()), "{owner}", Remain.getOfflinePlayerByUUID(turretData.getOwner()).getName(), "{level}", MathUtil.toRoman(turretData.getCurrentLevel()), "{health}", turretData.getCurrentHealth());
                    final Hologram hologram = DHAPI.getHologram(turretData.getId());
                    //turretData.getHologram().updateLore(loreHologram);
                    if (hologram != null)
                        DHAPI.setHologramLines(hologram, loreHologram);
                }
            }
